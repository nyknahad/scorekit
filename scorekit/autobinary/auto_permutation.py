...
#        return list(feat_after_corr)
    ...
    def depth_analysis(self, X_train: pd.DataFrame, y_train: pd.DataFrame, features:list=None, max_depth:int=5):
        ...
        for max_depth in tqdm_notebook(max_depth_grid):
            ...
            if self.model_type=='xgboost':
                fi.append(np.fromiter(model.get_booster().get_score(importance_type='gain').values(), dtype=float))
                fi_feat.append(np.fromiter(model.get_booster().get_score(importance_type='gain').values(), dtype=float))
